...
def find_cow(name,cows): #given a name and python list of cow objects, return the Cow object with the specified name
    iteration=0 #iteration variable is initialized
    for cowobject in cows: #for loop will go through each cow to see if the name matches
...
def main():
    ...
    if sys.argv[1]=="-l": #if -l is present in the sys list at the specified position, display the list of all available cows
    ...
    elif sys.argv[1]=="-n": #if -n is present in the sys list at the specified location, display the cow and message
        ...
        if cow!=None: #if a valid cow name is chosen, print the message and display the cow
            ...
            print(cow.get_image()) #after the specified cow is returned, display the cow
            if isinstance(cow,Dragon):  # checks to see whether or not the cow object is a dragon. if so, check to see whether or not it can breathe fire.
